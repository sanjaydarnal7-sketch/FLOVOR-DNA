...
const SciFiSlider: React.FC<SciFiSliderProps> = ({ label, name, value, min, max, step = 0.1, onChange, minLabel, maxLabel }) => {
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-1">
        <label className="text-sm font-medium text-gray-400 tracking-wider font-sans">{label}</label>
        <input
            type="number"
            name={name}
            value={String(value)}
            ...
            className="w-20 text-sm font-semibold font-mono text-[var(--primary-300)] bg-slate-900/50 px-2 py-0.5 rounded text-center border border-slate-700/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none focus:outline-none focus:ring-1 focus:ring-[var(--primary-400)] transition"
        />
      </div>
